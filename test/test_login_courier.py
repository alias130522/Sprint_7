...
class TestLoginCourier:
    ...
    @pytest.mark.parametrize('wrong_data', [
                                                {"login": data.fix_list_data[0], "password": 'wrong_pass'},
                                                {"login": 'wrong_login', "password": data.fix_list_data[1]},
                                                {"login": 'wrong_login', "password": 'wrong_pass'}])
    def test_courier_authorization_wrong(self, wrong_data):
        payload = {"login": data.fix_list_data[0], "password": data.fix_list_data[1]}
        requests.post(data.url_creating_courier, data=payload)
        payl = wrong_data
        response = requests.post(data.url_courier_authorization, data=payl)
